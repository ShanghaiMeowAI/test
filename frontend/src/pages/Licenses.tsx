import { useState, useEffect } from 'react';
import { Card } from '../components/ui/Card';
import { Button } from '../components/ui/Button';
import { StatusBadge } from '../components/ui/Badge';
import { Table, TableHeader, TableHeaderCell, TableBody, TableRow, TableCell, EmptyState } from '../components/ui/Table';
import { Loading, TableLoading } from '../components/ui/Loading';
import { LicenseForm } from '../components/licenses/LicenseForm';
import { licenseApi, customerApi } from '../services/api';
import type { License, Customer, LicenseFormData } from '../types';
import {
  PlusIcon,
  MagnifyingGlassIcon,
  KeyIcon,
  XMarkIcon,
  CheckIcon,
  EyeIcon,
  PencilIcon,
  TrashIcon,
  DocumentDuplicateIcon,
  CalendarIcon
} from '@heroicons/react/24/outline';

export function Licenses() {
  const [licenses, setLicenses] = useState<License[]>([]);
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('');
  const [typeFilter, setTypeFilter] = useState('');
  const [customerFilter, setCustomerFilter] = useState('');
  const [actionLoading, setActionLoading] = useState<number | null>(null);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    loadData();
  }, [searchTerm, statusFilter, typeFilter, customerFilter]);

  const loadData = async () => {
    try {
      setIsLoading(true);
      const [licensesResponse, customersResponse] = await Promise.all([
        licenseApi.getLicenses({
          search: searchTerm || undefined,
          status: statusFilter || undefined,
          license_type: typeFilter || undefined,
          customer: customerFilter ? parseInt(customerFilter) : undefined
        }),
        customerApi.getCustomers()
      ]);
      
      setLicenses(licensesResponse.data.results);
      setCustomers(customersResponse.data.results);
      setError(null);
    } catch (err) {
      setError('获取授权码列表失败');
      console.error('Error loading licenses:', err);
    } finally {
      setIsLoading(false);
    }
  };

  const handleActivate = async (licenseId: number) => {
    try {
      setActionLoading(licenseId);
      await licenseApi.activateLicense(licenseId, {});
      await loadData();
    } catch (err) {
      alert('激活授权码失败');
      console.error('Error activating license:', err);
    } finally {
      setActionLoading(null);
    }
  };

  const handleRevoke = async (licenseId: number) => {
    if (!window.confirm('确定要撤销这个授权码吗？此操作不可恢复。')) {
      return;
    }

    try {
      setActionLoading(licenseId);
      await licenseApi.revokeLicense(licenseId);
      await loadData();
    } catch (err) {
      alert('撤销授权码失败');
      console.error('Error revoking license:', err);
    } finally {
      setActionLoading(null);
    }
  };

  const handleDelete = async (licenseId: number) => {
    if (!window.confirm('确定要删除这个授权码吗？此操作不可恢复。')) {
      return;
    }

    try {
      await licenseApi.deleteLicense(licenseId);
      await loadData();
    } catch (err) {
      alert('删除授权码失败');
      console.error('Error deleting license:', err);
    }
  };

  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text);
    alert('授权码已复制到剪贴板');
  };

  const formatDate = (dateString?: string) => {
    if (!dateString) return '-';
    return new Date(dateString).toLocaleDateString('zh-CN');
  };

  const getStatusColor = (status: string): 'success' | 'warning' | 'danger' | 'default' => {
    switch (status) {
      case 'active': return 'success';
      case 'pending': return 'warning';
      case 'expired': return 'danger';
      case 'revoked': return 'danger';
      default: return 'default';
    }
  };

  const getTypeText = (type: string) => {
    const typeMap = {
      'trial': '试用版',
      'standard': '标准版',
      'professional': '专业版',
      'enterprise': '企业版'
    };
    return typeMap[type as keyof typeof typeMap] || type;
  };

  const handleCreateLicense = async (data: LicenseFormData) => {
    try {
      setIsSubmitting(true);
      await licenseApi.createLicense(data);
      setShowCreateForm(false);
      await loadData();
      alert('授权码生成成功');
    } catch (err) {
      alert('生成授权码失败');
      console.error('Error creating license:', err);
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="space-y-6">
      {showCreateForm ? (
        <LicenseForm
          onSubmit={handleCreateLicense}
          onCancel={() => setShowCreateForm(false)}
          isSubmitting={isSubmitting}
        />
      ) : (
        <>
          {/* 页面头部 */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">授权码管理</h1>
              <p className="mt-2 text-sm text-gray-700">
                管理离线部署的授权码
              </p>
            </div>
            <div className="mt-4 sm:mt-0">
              <Button onClick={() => setShowCreateForm(true)}>
                <PlusIcon className="h-4 w-4 mr-2" />
                生成授权码
              </Button>
            </div>
          </div>

          {/* 搜索和筛选 */}
          <Card>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {/* 搜索框 */}
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <MagnifyingGlassIcon className="h-5 w-5 text-gray-400" />
                </div>
                <input
                  type="text"
                  placeholder="搜索授权码、域名或客户..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>

              {/* 状态筛选 */}
              <div>
                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="block w-full px-3 py-2 border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="">所有状态</option>
                  <option value="active">有效</option>
                  <option value="pending">待激活</option>
                  <option value="expired">已过期</option>
                  <option value="revoked">已撤销</option>
                </select>
              </div>

              {/* 类型筛选 */}
              <div>
                <select
                  value={typeFilter}
                  onChange={(e) => setTypeFilter(e.target.value)}
                  className="block w-full px-3 py-2 border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="">所有类型</option>
                  <option value="trial">试用版</option>
                  <option value="standard">标准版</option>
                  <option value="professional">专业版</option>
                  <option value="enterprise">企业版</option>
                </select>
              </div>

              {/* 客户筛选 */}
              <div>
                <select
                  value={customerFilter}
                  onChange={(e) => setCustomerFilter(e.target.value)}
                  className="block w-full px-3 py-2 border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="">所有客户</option>
                  {customers.map((customer) => (
                    <option key={customer.id} value={customer.id}>
                      {customer.name}
                    </option>
                  ))}
                </select>
              </div>
            </div>
          </Card>

          {/* 授权码列表 */}
          <Card padding="none">
            {isLoading ? (
              <TableLoading />
            ) : error ? (
              <div className="text-center py-12">
                <p className="text-red-600">{error}</p>
                <Button onClick={loadData} className="mt-4">
                  重试
                </Button>
              </div>
            ) : licenses.length === 0 ? (
              <EmptyState
                title="暂无授权码"
                description="还没有生成任何授权码"
                action={
                  <Button onClick={() => setShowCreateForm(true)}>
                    <PlusIcon className="h-4 w-4 mr-2" />
                    生成第一个授权码
                  </Button>
                }
              />
            ) : (
              <Table>
                <TableHeader>
                  <TableHeaderCell>授权码信息</TableHeaderCell>
                  <TableHeaderCell>客户</TableHeaderCell>
                  <TableHeaderCell>类型</TableHeaderCell>
                  <TableHeaderCell>状态</TableHeaderCell>
                  <TableHeaderCell>资源限制</TableHeaderCell>
                  <TableHeaderCell>有效期</TableHeaderCell>
                  <TableHeaderCell className="text-right">操作</TableHeaderCell>
                </TableHeader>
                <TableBody>
                  {licenses.map((license) => (
                    <TableRow key={license.id}>
                      <TableCell>
                        <div>
                          <div className="flex items-center space-x-2">
                            <code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded">
                              {license.license_key.slice(0, 20)}...
                            </code>
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => copyToClipboard(license.license_key)}
                            >
                              <DocumentDuplicateIcon className="h-4 w-4" />
                            </Button>
                          </div>
                          {license.deployment_domain && (
                            <div className="text-sm text-gray-500 mt-1">
                              部署域名: {license.deployment_domain}
                            </div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm font-medium text-gray-900">
                          {license.customer_name}
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="text-sm text-gray-900">
                          {getTypeText(license.license_type)}
                        </span>
                      </TableCell>
                      <TableCell>
                        <StatusBadge status={license.status} />
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          <div>用户: {license.max_users}</div>
                          <div>公司: {license.max_companies}</div>
                          <div>存储: {license.max_storage_gb}GB</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          <div>生效: {formatDate(license.valid_from)}</div>
                          <div>到期: {formatDate(license.valid_until)}</div>
                          {license.status === 'active' && (
                            <div className="text-green-600">
                              剩余: {license.days_remaining}天
                            </div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end space-x-2">
                          {license.status === 'pending' && (
                            <Button
                              size="sm"
                              variant="success"
                              onClick={() => handleActivate(license.id)}
                              loading={actionLoading === license.id}
                            >
                              <CheckIcon className="h-4 w-4" />
                            </Button>
                          )}
                          {(license.status === 'active' || license.status === 'pending') && (
                            <Button
                              size="sm"
                              variant="danger"
                              onClick={() => handleRevoke(license.id)}
                              loading={actionLoading === license.id}
                            >
                              <XMarkIcon className="h-4 w-4" />
                            </Button>
                          )}
                          <Button size="sm" variant="ghost">
                            <EyeIcon className="h-4 w-4" />
                          </Button>
                          <Button size="sm" variant="ghost">
                            <PencilIcon className="h-4 w-4" />
                          </Button>
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => handleDelete(license.id)}
                          >
                            <TrashIcon className="h-4 w-4 text-red-500" />
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            )}
          </Card>
        </>
      )}
    </div>
  );
}
